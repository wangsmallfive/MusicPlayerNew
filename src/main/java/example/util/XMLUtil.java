...
public class XMLUtil {
    ...
    static {
        ...
        if (!file.exists()) {
            ...
            try {
                ...
                root.addElement("song-menu").addAttribute("songMenuName", "服务器音乐").addAttribute("createDate", time);
        ...
        File file2 = new File("E://MusicPlay//");
        if (!file.exists()) {
            file.mkdirs();
            ...
            try {
                ...
                DateFormat format = new SimpleDateFormat("yyyy年MM月dd日");
                String time = format.format(date);
                root.addElement("song-menu").addAttribute("songMenuName", "我服务器的音乐").addAttribute("createDate", time);
    ...
    // 歌单默认保存目录E://MusicPlay//songsheet.xml，后面提供方法修改
    public static Document getServerDoc() {
        try {
            File songMenuXML = new File("E://MusicPlay//serversongmenu.xml");
            if (!songMenuXML.exists()) {
                try {
            ...
            Document read = new SAXReader().read(new File("E://MusicPlay//serversongmenu.xml"));
            return read;
        } catch (DocumentException e) {
            e.printStackTrace();
        }
    ...
    public static void writeDocServer(Document document) {
        XMLWriter xmlWriter = null;
        try {
            FileOutputStream fileOutputStream = new FileOutputStream(new File("E://MusicPlay//serversongmenu.xml"));
            OutputFormat outputFormat = OutputFormat.createPrettyPrint();
            xmlWriter = new XMLWriter(fileOutputStream, outputFormat);
            xmlWriter.write(document);
        } catch (Exception e) {
