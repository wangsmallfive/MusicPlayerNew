...
import example.controller.Controller.ContentController;
import example.controller.Controller.StackController;
import example.controller.Page.SearchPage;
...
public class MainAction {
    ...
    public boolean back() {
        ...
        IntegerProperty i = gui.getIndex();
        IntegerProperty s = gui.getSize();
        int ii = i.get();
        if (ii < 1)
            return false;
        Page p;
    ...
    public void onupDownlodeMusic() {
        if (pq.getSize() > 0 && pq.getPage() instanceof Page.UpDownlodeLocalPage)
            return;
        currentMenu.set("服务器音乐");
        Page p = giveUpDownLocal();
        pq.add(p);
        show(p);
    }
    //刷新服务器
    ...
    private static void scrollLrc(int totalSec, double progress) {
        ...
        for (int i = 0; i < lyric.size(); i++) {
            ...
            if (ld.type != LrcAnalyzer.LRC_ZONE)
                continue;
            if (i - 1 >= 0) pm = lyric.get(i - 1).timeMs;
            else pm = 0;
    ...
    private Page giveLocal() {
        ...
        try {
            FXMLLoader lm = new FXMLLoader(GUI.class.getResource("../../fxml/LocalMusicPage.fxml"),
    ...
    private Page giveUpDownLocal() {
        ServerMusicPageController smC = null;
        StackPane upDownMusic = null;
        try {
            FXMLLoader lm = new FXMLLoader(GUI.class.getResource("../../fxml/ServerMusicPage.fxml"),
                    ResourceBundle.getBundle("ini"));
            upDownMusic = (StackPane) lm.load();
            upDownMusic.getChildren().add(gui.getPlaylist());
            smC = lm.getController();
            List<MusicUtils> list = new ArrayList<MusicUtils>();
            smC.initData(this, list);
            List<MusicUtils> ml = SongMenuOperate.getServerSong();
           // List<MusicUtils> ml = SongMenuOperate.getServerlSong();




            smC.getTableView_musicTable2().getItems().addAll(ml);
            smC.getLabel_musicNum().setText(ml.size() + "首音乐");
        } catch (IOException e) {
            e.printStackTrace();
        }
        return Page.newPage(Controller.SERVER, upDownMusic, smC);
    }
    private Page giveMusicList(String name, String date) {
        ...
        try {
            FXMLLoader ml = new FXMLLoader(GUI.class.getResource("../../fxml/MusicListPage.fxml"),
                    ResourceBundle.getBundle("ini"));
    ...
    private Page giveSearch(String key) {
        ...
        try {
            FXMLLoader sp = new FXMLLoader(GUI.class.getResource("../../fxml/SearchPage.fxml"), ResourceBundle.getBundle("ini"));
            searchpage = (StackPane) sp.load();
            searchpage.getChildren().add(gui.getPlaylist());
            spC = sp.getController();
            List<MusicUtils> list = new ArrayList<MusicUtils>();
            spC.initData(this, key, list);
            List<MusicUtils> sl = searchsong(key, SEARCHMODE_SONGNAME,"local");
    ...
    public static List<MusicUtils> searchsong(String key, int mode,String where ) {
        List<MusicUtils> ml = new ArrayList<>();
        List<Song> sl;
        if (!key.equals("")) {
            sl = SongOperate.findSongByName(key, currentMenu.get(), mode,where);
        } else {
            if (where == "local"){
                if (PageType == Controller.LOCAL)
                ...
                if (PageType == Controller.LOCAL)
                    return SongMenuOperate.getServerSong();
                else
                    return SongMenuOperate.getServerSongsByMenuName(currentMenu.get());
            }
    ...
    public MainAction(GUI gui) {
        ...
        tsca = new TagServerClickAction(this, gui.getLlC().getListView_musicList());
        ...
        btn.getStyleClass().remove(0);
        btn.getStyleClass().add("AddList");
    ...
    public TagServerClickAction tsca;
    ...
    public final static Image defaultEmptyImage = new Image("example/css/left/demo.png");
