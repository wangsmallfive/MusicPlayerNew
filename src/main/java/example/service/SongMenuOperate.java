...
import example.dao.SongMenuDao;
import example.dao.impl.SongMenuDaoImpl;
...
public class SongMenuOperate {
    ...
    //通过歌单名得到歌单下的歌
    public static List<MusicUtils> getServerSongsByMenuName(String menuName) {
        List<Song> songByMenuName = songMenuDao.getServerSongByMenuName(menuName);
        List<MusicUtils> songUtilMenu = new ArrayList<MusicUtils>();
        Iterator<Song> iterator = songByMenuName.iterator();
        while (iterator.hasNext()) {
            Song next = iterator.next();
    ...
    //返回本地音乐
    public static List<MusicUtils> getServerSong() {
        List<Song> songByMenuName = songMenuDao.getServerSongByMenuName("服务器音乐");
        List<MusicUtils> songUtilMenu = new ArrayList<MusicUtils>();
        Iterator<Song> iterator = songByMenuName.iterator();
        while (iterator.hasNext()) {
            Song next = iterator.next();
